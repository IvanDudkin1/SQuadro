var tagsList = {
    ...
    getColumnsDefinition: function () {
        var actions = function (data, type, row) {
            var id = row.ID;
            var actions = [];
            if (!tagsList.listtemplate.readonly) {
                actions.push({ name: "Edit", click: "tagsList.edit('" + id + "')", html: '<i class="glyphicon glyphicon-edit"></i>' });
                actions.push({ name: "Delete", click: "tagsList.delete('" + id + "','" + row.Name + "')", html: '<i class="glyphicon glyphicon-remove"></i>' });
            }
    ...
    addNew: function () {
        tagsList.edit(null, true);
    },

    edit: function (id, addNew) {
        var uriDetails = tagsList.listtemplate.root + 'Tags/GetTagDetails';
        ...
        $.ajax({
            ...
            success: function (data) {
                var modalName = addNew ? "Add New Tag" : "Edit Tag";
    ...
    'delete': function (id, name) {
        var uriDelete = tagsList.listtemplate.root + 'Tags/Delete';
        var $modal = modalService.createModal("Delete Tag", { Cancel: true, Delete: { callback: function () {
             $.ajax({
                cache: false,
        ...
        $('.modal-body', $modal).html("<h4>Are you sure to delete Tag " + name + "?</h4>");
        $modal.modal();
    }
}