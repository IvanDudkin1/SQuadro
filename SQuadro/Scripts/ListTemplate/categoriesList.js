var categoriesList = {
    ...
    getColumnsDefinition: function () {
        var actions = function (data, type, row) {
            var id = row.ID;
            var actions = [];
            if (!categoriesList.listtemplate.readonly) {
                actions.push({ name: "Edit", click: "categoriesList.edit('" + id + "')", html: '<i class="glyphicon glyphicon-edit"></i>' });
                actions.push({ name: "Delete", click: "categoriesList.delete('" + id + "','" + row.Name + "')", html: '<i class="glyphicon glyphicon-remove"></i>' });
            }
    ...
    addNew: function () {
        categoriesList.edit(null, true);
    },

    edit: function (id, addNew) {
        var uriDetails = categoriesList.listtemplate.root + 'Categories/GetCategoryDetails';
        ...
        $.ajax({
            ...
            success: function (data) {
                var modalName = addNew ? "Add New Category" : "Edit Category";
                var $modal = modalService.createModal(modalName, { Cancel: true, Save: { callback: function () {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            {
                                categoriesList.listtemplate.refresh();
                                $modal.modal('hide');
                            }
                        }
                    });
    ...
    delete: function (id) {
        var uriDelete = categoriesList.listtemplate.root + 'Categories/Delete';
        var $modal = modalService.createModal("Delete Category", { Cancel: true, Delete: { callback: function () {
             $.ajax({
                cache: false,
        ...
        $('.modal-body', $modal).html("<h4>Are you sure to delete the Category?</h4>");
        $modal.modal();
    }
}